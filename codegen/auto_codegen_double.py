...
def compile_program():
    ...
    subprocess.run("make -C ../ maind >& ../output.txt", shell=True, check=False)
...
def run_program():
    try:
        subprocess.run("../maind > ../output1.txt", shell=True, check=False)
        if check_res():
            return True
        return False
    except:
...
def create_function(item, num):
    myfunc = f"""
template <typename ElementA_, int kAlignmentA, typename ElementB_, int kAlignmentB, typename ElementC_, typename ElementAccumulator, typename EpilogueOutputOp, int Stages, bool isRowMajor> struct GEMM_double_{num} {{
...
void cutlassFusedDistanceNN_double_{num}(const DataT* x,const DataT* y,const DataT* xn,const DataT* yn,IdxT m,IdxT n,IdxT k,IdxT lda,IdxT ldb,IdxT ldd,OutT* dOutput,int* mutexes,CGReduceOpT cg_reduce_op,DistanceFn dist_op,ReduceOpT redOp,KVPReduceOpT pairRedOp,cudaStream_t stream){{
  ...
  using fusedDistanceNNKernel = typename cutlass::gemm::kernel::GEMM_double_{num}<DataT,Alignment,DataT,Alignment,AccT,AccT,EpilogueOutputOp,NumStages,isRowMajor>::GemmKernel;
...
def create_ifs(num):
    myif = f"""
    if (num == {num}) cutlassFusedDistanceNN_double_{num}<DataT,DataT,OutT,IdxT,16 / sizeof(DataT),kvp_cg_min_reduce_op_,L2Op,MinAndDistanceReduceOp,KVPMinReduce>(dx,dy,dxn,dyn,m,n,k,lda,ldb,ldd,dmin,dworkspace,cg_reduce_op,L2_dist_op,redOp,pairRedOp,stream);
"""
    return myif
...
def write_kernel(func):
    ...
    y = 0
    for i, line in enumerate(lines):
        if "//double start of injection" in line:
            x = i
        if "//double end of injection" in line:
            y = i
            break
...
def write_ifs(ifs):
    Fi = "../src/test_kmeans_tuner_double.cu"
    with open(Fi, "r") as file:
        lines = file.readlines()
    x = 0
    y = 0
